...
import torch
import numpy as np
from cg_baseline.contact_grasp_estimator import GraspEstimator
from cg_baseline import config_utils

from cg_baseline.checkpoints import CheckpointIO 
from cg_baseline.data import load_available_input_data
...
def inference(global_config, 
    ...
    for p in glob.glob(input_paths):
        ...
        # Save results
        np.savez('./contact_graspnet_results/predictions_{}'.format(os.path.basename(p.replace('png','npz').replace('npy','npz'))), 
