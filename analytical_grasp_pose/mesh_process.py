...
def collision_test(mesh, gripper_points, threshold):
    ...
    if dist_min < threshold:
        ...
        return False
def collision_test_local(mesh, gripper_points, grasp_pose, gripper_attr, threshold):
    '''
    The function to check whether the gripper collides with the object mesh
    ...
    pt1 = np.array([length*1.5, thickness*1.5, width*1.5])
    pt2 = np.array([-length*1.5, thickness*1.5, width*1.5])
    pt3 = np.array([-length*1.5, -thickness*1.5, width*1.5])
    pt4 = np.array([length*1.5, -thickness*1.5, width*1.5])
    ...
    # Transform the box to the grasp pose
    box = np.vstack((box.T, np.ones((1, box.shape[0]))))
    box = np.matmul(grasp_pose, box)[:-1].T
