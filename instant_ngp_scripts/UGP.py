...
if __name__ == "__main__":
    ...
    # In the first round, initialize a nerf scene from only 10 images
    initial_pose_idx = []
    initial_poses = []
    for idx in range(0, 30, 5):
         camera_pose_candidates[2*i_num + idx]["selected"] = True
         initial_poses.append(camera_pose_candidates[2*i_num + idx])
    # Take the initial ten images
    ugp_pipeline.pyrender_take_snapshot(initial_poses, pyrender_args)

    ugp_pipeline.init_testbed(testbed_args=testbed_args)
    # Train the nerf model
    ugp_pipeline.train_NeRF(testbed_args)
    ...
    for iter in range(iter_num):
        ...
        # TODO: start training from an existing pre-trained model
        ugp_pipeline.init_testbed(testbed_args=testbed_args)
    ...
    if testbed_args.ugp_debug:
        ...
        entropy_pose1["render_image_name"] = "render_test_" + "final" + ".png"
        entropy_pose1["fov"] = 90
        entropy_poses.append(entropy_pose1)
        # Look at the entropy image as a reference
